...
public abstract class DockerFixtureBase<TProgram> : BaseFixture, IAsyncLifetime, IFixture where TProgram : class
{
	private readonly IMessageSink _messageSink;
	protected readonly Dictionary<string, DockerContainer> DockerContainers;
	protected DockerFixtureBase(IMessageSink messageSink, Dictionary<string, DockerContainer> dockerContainers)
	{
	...
	{
		var dockerTasks = DockerContainers.Select(container => container.Value.StartAsync()).ToArray();
	...
	public virtual async Task DisposeAsync()
	{
		Client.Dispose();
		await TearDownAsync();
		var dockerTasks = DockerContainers.Select(container => container.Value.StopAsync()).ToArray();
		Task.WaitAll(dockerTasks);
	}
}
